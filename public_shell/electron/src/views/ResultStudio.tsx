...
export function ResultStudio() {
  ...
  useEffect(() => { setImgError(false); }, [template]);
  ...
  const fetchMetadata = async () => {
    ...
    try {
      ...
      const tchRes = await window.electronAPI.getAllTeachers({ limit: 500, minimal: true });
      setTeachers(tchRes?.data || []);

      const studRes = await window.electronAPI.getAllStudents({ limit: 5000, minimal: true });
  ...
  const handlePreview = async () => {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        setQueryMessage(`${resp.results?.length || 0} student(s) · ${session}, ${term}`);
      } else {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (res && res.success) {
        const fmtLabel: Record<string, string> = { pdf: 'PDF', html: 'HTML file', image: 'PNG image' };
  ...
  const handleCopyImage = async () => {
    ...
    try {
      const res = await window.electronAPI.copyResultImage({ imagePath: lastImagePath });
  ...
  const handleAutoFillRemarks = () => {
    const isEndTerm = currentTerm.toLowerCase().includes('third') || currentTerm.toLowerCase().includes('3rd');
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div className="view-header" style={{ flexShrink: 0 }}>
        <div>
          ...
          <p className="view-sub">Generate, preview and export result cards in any format.</p>
        </div>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          {genStatus && (
            <span style={{
              fontSize: '12px',
              color: genStatus.startsWith('✅') ? 'var(--accent)' : genStatus.startsWith('❌') ? 'var(--danger)' : 'var(--text-dim)'
            }}>
              {genStatus}
            </span>
          )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '22px 20px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
            ...
            {PAID_TEMPLATES.includes(template) && reportType !== 'portal_card' && (
              <div style={{
                display: 'flex',
                alignItems: 'center',
                gap: '8px',
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '11.5px', color: 'var(--text-dim)' }}>
                  <input
                    type="checkbox"
                ...
                {useBrandColors && (
                  <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                    <div style={{ width: '20px', height: '20px', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.15)', background: brandPrimary }} />
                    <div style={{ width: '20px', height: '20px', borderRadius: '4px', border: '1px solid rgba(255,255,255,0.15)', background: brandSecondary }} />
                  </div>
                )}
              </div>
            )}
            ...
            <div className="ph-config-group">
              ...
              >
                ...
                <option value="portal_card">🔐 Parent Portal Access Cards</option>
              </select>
            </div>
            ...
            {scope === 'class' && (
              <div className="ph-config-group">
                ...
                <select className="modern-input" style={{ width: '100%' }} value={selectedClass} onChange={e => setSelectedClass(e.target.value)}>
            ...
            {scope === 'teacher' && (
              <div className="ph-config-group">
                ...
                <select className="modern-input" style={{ width: '100%' }} value={selectedTeacherId} onChange={e => setSelectedTeacherId(e.target.value)}>
                  <option value="">— Select —</option>
            ...
            {scope === 'subject' && (
              <div className="ph-config-group">
                ...
                <select className="modern-input" style={{ width: '100%' }} value={selectedSubject} onChange={e => setSelectedSubject(e.target.value)}>
                  <option value="">— Select —</option>
            ...
            {scope === 'student' && (
              <div className="ph-config-group">
                ...
                <select className="modern-input" style={{ width: '100%' }} value={selectedStudentId} onChange={e => setSelectedStudentId(e.target.value)}>
                  <option value="">— Select —</option>
          ...
          <div style={{
            ...
            {generatedPath && (
              <span style={{ fontSize: '10px', color: 'var(--text-dim)', wordBreak: 'break-all' }}>{generatedPath}</span>
            )}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            <span className="ph-label" style={{ fontSize: '10px', textTransform: 'uppercase', letterSpacing: '1.5px', opacity: 0.55, flexShrink: 0, alignSelf: 'center' }}>
              📋 Template Preview
            </span>
            <div style={{
              width: '100%',
              ...
              borderRadius: '8px',
              border: imgError ? '1px dashed rgba(255,255,255,0.15)' : '1px solid rgba(255,255,255,0.06)',
              overflow: 'hidden',
              position: 'relative',
              display: 'flex',
              alignItems: 'center',
              ...
              {imgError ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', color: 'rgba(255,255,255,0.3)' }}>
                  ...
                  <span style={{ fontSize: '10px', textAlign: 'center', lineHeight: 1.4 }}>Preview image<br />not yet available</span>
                </div>
              ) : (
                <img
                  src={templateImgSrc}
                  ...
                  style={{ width: '100%', height: '100%', objectFit: 'cover', objectPosition: 'top center', display: 'block', borderRadius: '3px' }}
                />
              )}
            </div>
            <span style={{ fontSize: '10px', color: 'var(--text-dim)', textAlign: 'center', lineHeight: 1.6, flexShrink: 0, opacity: 0.7 }}>
          ...
          {previewActive && (
            <div style={{ flex: 1, overflowY: 'auto', borderTop: '1px solid var(--glass-border)' }}>
              <div style={{ padding: '14px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span id="rs-preview-label" style={{ fontSize: '12px', color: 'var(--text-dim)' }}>{queryMessage}</span>
              </div>
              <div className="table-container" id="rs-preview-container" style={{ margin: '0 0 16px' }}>
                    ...
                    {queryResults.length === 0 ? (
                      <tr>
                        <td colSpan={6} style={{ textAlign: 'center', padding: '30px', color: 'var(--text-dim)' }}>
                          {queryMessage.includes('Querying') ? '⏳ Loading…' : 'No results found. Ensure grades have been synced from teacher devices.'}
                        </td>
                      </tr>
                    ) : (
                      queryResults.map((s, i) => (
                        <tr key={s.id}>
                          ...
                          <td>{s.class_name}</td>
                          <td>{s.subjects?.filter((x: any) => x.score !== null).length || 0} graded</td>
                          <td>{s.total_score ?? '—'}</td>
                          <td>{s.average ?? '—'}</td>
                        </tr>
                      ))
        ...
        <div style={{ position: 'fixed', inset: 0, zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0, 0, 0, 0.75)', backdropFilter: 'blur(8px)', userSelect: 'none', WebkitAppRegion: 'no-drag' } as React.CSSProperties}>
          <div style={{ background: 'var(--bg-dark)', border: '1px solid var(--glass-border)', borderRadius: 'var(--radius-xl)', width: '90%', maxWidth: '950px', height: '82vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0, 0, 0, 0.5)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '20px 24px' }}>
              {remarksSaveStatus && (
                <div style={{ marginBottom: '14px', background: 'rgba(0, 229, 255, 0.1)', border: '1px solid rgba(0, 229, 255, 0.25)', borderRadius: 'var(--radius-sm)', padding: '10px 16px', fontSize: '12px', color: 'var(--accent)' }}>
                  {remarksSaveStatus}
                </div>
              )}
                      ...
                      <tr key={student.id}>
                        ...
                        <td>
                          <textarea
                            ...
                            onChange={e => setRemarksData(prev => prev.map((s, i) => i === idx ? { ...s, remark: e.target.value } : s))}
                            className="modern-input"
                            style={{ width: '100%', height: '56px', resize: 'vertical', fontSize: '12px', padding: '6px 10px' }}
                        ...
                        <td>
                          <textarea
                            ...
                            onChange={e => setRemarksData(prev => prev.map((s, i) => i === idx ? { ...s, principal_remark: e.target.value } : s))}
                            className="modern-input"
                            style={{ width: '100%', height: '56px', resize: 'vertical', fontSize: '12px', padding: '6px 10px' }}
